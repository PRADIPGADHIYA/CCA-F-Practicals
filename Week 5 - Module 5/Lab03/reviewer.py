...
Demo 2 TODOs are filled in: tolerant JSON parse + local-quote attachment.
"""
...
def _attach_quotes(findings: list) -> list[dict]:
    """Attach provenance locally. The quote comes from OUR data, never the model."""
    cleaned = []
    for f in findings:
...
def review(report_text: str, mode: str = "strict") -> list[dict]:
    ...
    return _attach_quotes(findings)
