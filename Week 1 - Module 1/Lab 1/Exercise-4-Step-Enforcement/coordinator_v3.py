"""
Exercise 4 — Step 2: coordinator with gates wired between every step.

Identical pipeline to coordinator_v2.py (Exercise 3), but each step is
now followed by a programmatic gate that must pass before the next step
is allowed to run. If a gate fails, PipelineGateError is caught and the
pipeline stops cleanly with a clear [PIPELINE BLOCKED] message instead of
